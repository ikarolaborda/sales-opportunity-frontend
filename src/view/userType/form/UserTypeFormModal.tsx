...
import UserTypeForm from 'src/view/userType/form/UserTypeForm';
import UserTypeService from 'src/modules/userType/userTypeService';

function UserTypeFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await UserTypeService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.userType.new.title')}
            </h5>
            <button
              type="button"
              className="close"
